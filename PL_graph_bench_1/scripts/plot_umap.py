"""
Plot UMAP visualizations for different graph building methods
"""
import sys
...
# Find all UMAP embeddings (look for UMAP keys in obsm)
...
# Also check if params.umap_keys is provided
if hasattr(params, 'umap_keys') and params.umap_keys:
    # Filter to only include keys that exist
    for key in params.umap_keys:
        if key in adata.obsm and key not in umap_keys_available:
            umap_keys_available.append(key)
            print(f"  {key}: shape {adata.obsm[key].shape}")
...
# Sort UMAP keys for consistent ordering
umap_keys_available = sorted(umap_keys_available)
...
for i, umap_key in enumerate(umap_keys_available):
    ...
    if 'X_umap_dif_' in umap_key:
        ...
        # Try to extract any method name pattern
        display_name = umap_key
