# PL_hyperp: k_mnn and n_edges_per_node Hyperparameter Optimization Pipeline

This Snakemake pipeline tests different `k_mnn` values for CellDiffusion integration, with `n_edges_per_node` set equal to `k_mnn` for each test, and evaluates the results using scib-metrics.
...
3. Builds integration graphs with different `k_mnn` values (10, 20, 30, 40, 50), where `n_edges_per_node` = `k_mnn` for each test
...
**For each `k_mnn` value, `n_edges_per_node` is automatically set equal to `k_mnn`**. This allows testing the effect of both parameters together when they are balanced.
...
- `n_edges_per_node` is automatically set equal to `k_mnn` (no need to configure separately)
- UMAP parameters
...
```
preprocess → encode_features → build_graph (for each k_mnn, n_edges_per_node=k_mnn) → integrate (for each k_mnn)
                                                                                          ↓
...
- `graph_kmnn{k_mnn}.h5ad` - Integration graphs for each k_mnn (with n_edges_per_node=k_mnn)
...
- **k_mnn**: Number of mutual nearest neighbors for inter-batch connections (varied: [10, 20, 30, 40, 50])
- **n_edges_per_node**: Maximum number of outgoing edges per node (automatically set equal to k_mnn)
...
- For each k_mnn, n_edges_per_node is automatically set to the same value
