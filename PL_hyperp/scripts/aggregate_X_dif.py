"""
Aggregate X_dif from all k_mnn integration outputs into a single adata file
"""
import sys
...
# Extract k_mnn values from filenames and store X_dif
x_dif_dict = {}
...
for h5ad_file in input_h5ad_files:
    ...
    # Extract k_mnn value from filename (format: integrated_kmnn{k_mnn}.h5ad)
    filename = Path(h5ad_file).name
    
    # Try to extract k_mnn from filename
    match = re.search(r'kmnn(\d+)', filename)
    if match:
        k_mnn = int(match.group(1))
    else:
        print(f"Warning: Could not extract k_mnn from filename {filename}, skipping")
        continue
    ...
    # Store X_dif with key indicating k_mnn value
    key_name = f"X_dif_kmnn{k_mnn}"
...
# Also add a metadata column to track which k_mnn values were aggregated
base_adata.uns['aggregated_X_dif'] = {
    'k_mnn_values': sorted([int(k.split('kmnn')[1]) for k in x_dif_dict.keys()]),
...
print(f"  k_mnn values: {base_adata.uns['aggregated_X_dif']['k_mnn_values']}")
