...
Square subplots for each UMAP visualization
"""
import sys
...
# Calculate figure size with square subplots
subplot_size = 5  # Size of each square subplot (in inches)
fig_width = subplot_size * n_cols + 1.5  # Add extra width for spacing
fig_height = subplot_size * n_layers + 2  # Height for UMAP rows + legend row

fig = plt.figure(figsize=(fig_width, fig_height))
gs = GridSpec(n_rows, n_cols, figure=fig,
              hspace=0.35, wspace=0.35,  # Spacing between subplots
        ...
        try:
            ...
            if umap_key_dif in adata.obsm:
                ...
                sc.pl.umap(
                    ...
                    frameon=False,
                    legend_loc='none',
                    title=f"Layer {layer} - CellDiffusion Batch" if layer_idx == 0 else None,
                    title_fontsize=14
                )
                axes[layer_idx][0].title.set_fontsize(13)
                ...
                sc.pl.umap(
                    ...
                    frameon=False,
                    legend_loc='none',
                    title=f"Layer {layer} - CellDiffusion Cell Type" if layer_idx == 0 else None,
                    title_fontsize=14
                )
                axes[layer_idx][1].title.set_fontsize(13)
                
                # Clean up
                del adata.obsm['X_umap']
            else:
                print(f"    Warning: {umap_key_dif} not found in adata")
                axes[layer_idx][0].text(0.5, 0.5, 'Data not available', ha='center', va='center')
                axes[layer_idx][1].text(0.5, 0.5, 'Data not available', ha='center', va='center')
        except Exception as e:
            print(f"    Error plotting CellDiffusion: {e}")
            import traceback
            traceback.print_exc()
            axes[layer_idx][0].text(0.5, 0.5, 'Error', ha='center', va='center')
            axes[layer_idx][1].text(0.5, 0.5, 'Error', ha='center', va='center')
    else:
        print(f"  Warning: CellDiffusion data not found for layer {layer}")
        axes[layer_idx][0].text(0.5, 0.5, 'Not available', ha='center', va='center')
        axes[layer_idx][1].text(0.5, 0.5, 'Not available', ha='center', va='center')
        ...
        try:
            ...
            if umap_key_gcn in adata.obsm:
                ...
                sc.pl.umap(
                    ...
                    frameon=False,
                    legend_loc='none',
                    title=f"Layer {layer} - GCN Batch" if layer_idx == 0 else None,
                    title_fontsize=14
                )
                axes[layer_idx][2].title.set_fontsize(13)
                ...
                sc.pl.umap(
                    ...
                    frameon=False,
                    legend_loc='none',
                    title=f"Layer {layer} - GCN Cell Type" if layer_idx == 0 else None,
                    title_fontsize=14
                )
                axes[layer_idx][3].title.set_fontsize(13)
                
                # Clean up
                del adata.obsm['X_umap']
            else:
                print(f"    Warning: {umap_key_gcn} not found in adata")
                axes[layer_idx][2].text(0.5, 0.5, 'Data not available', ha='center', va='center')
                axes[layer_idx][3].text(0.5, 0.5, 'Data not available', ha='center', va='center')
        except Exception as e:
            print(f"    Error plotting GCN: {e}")
            import traceback
            traceback.print_exc()
            axes[layer_idx][2].text(0.5, 0.5, 'Error', ha='center', va='center')
            axes[layer_idx][3].text(0.5, 0.5, 'Error', ha='center', va='center')
    else:
        print(f"  Warning: GCN data not found for layer {layer}")
        axes[layer_idx][2].text(0.5, 0.5, 'Not available', ha='center', va='center')
        axes[layer_idx][3].text(0.5, 0.5, 'Not available', ha='center', va='center')
...
# Columns 0 and 1: Batch legend (shared, spanning first 2 columns)
# Columns 2 and 3: Cell Type legend (shared, spanning last 2 columns)
...
if batch_handles and batch_labels:
    ...
    batch_legend = fig.legend(batch_handles, batch_labels,
                              ...
                              ncol=4,  # Multiple columns for batch legend
                              bbox_to_anchor=(0.08, 0.015),
                              title='Batch', title_fontsize=12,
                              framealpha=0.95)
    batch_legend.get_frame().set_edgecolor('black')
else:
    legend_axes[0].axis('off')
    legend_axes[1].axis('off')
...
if label_handles and label_labels:
    ...
    label_legend = fig.legend(label_handles, label_labels,
                              loc='lower center',
                              frameon=True, fontsize=11,
                              ncol=6,  # Multiple columns for cell type legend
                              bbox_to_anchor=(0.65, 0.015),
                              title='Cell Type', title_fontsize=12,
                              framealpha=0.95)
    label_legend.get_frame().set_edgecolor('black')
else:
    legend_axes[2].axis('off')
    legend_axes[3].axis('off')
