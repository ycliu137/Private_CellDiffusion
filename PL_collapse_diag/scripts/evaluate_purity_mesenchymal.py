...
# Determine which representation to use based on method and network layers
if params.method == "CellDiffusion":
    use_rep = f'X_dif_nsteps{params.network_layers}'
    print(f"Using representation: {use_rep}")
elif params.method == "GCN":
    use_rep = f'X_gcn_nlayers{params.network_layers}'
    print(f"Using representation: {use_rep}")
else:
