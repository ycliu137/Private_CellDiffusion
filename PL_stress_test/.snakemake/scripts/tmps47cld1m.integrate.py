...
# Get k_value from params (passed from Snakefile)
k_value = getattr(params, 'k_value', params.loss_adj_k)
# Write metrics with header (each k value writes to its own file)
with open(metrics_csv, mode="w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["loss_adj_k", "k_added", "neighbor_purity_before", "neighbor_purity_after"])
    writer.writerow([k_value, params.k_add, neighbor_purity_before, neighbor_purity_after])
