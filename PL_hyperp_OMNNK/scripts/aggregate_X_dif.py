"""
Aggregate X_dif from all n_edges_per_node integration outputs into a single adata file
"""
import sys
...
# Extract n_edges_per_node values from filenames and store X_dif
x_dif_dict = {}
...
for h5ad_file in input_h5ad_files:
    ...
    # Extract n_edges_per_node value from filename (format: integrated_nedges{n_edges_per_node}.h5ad)
    filename = Path(h5ad_file).name
    
    # Try to extract n_edges_per_node from filename
    match = re.search(r'nedges(\d+)', filename)
    if match:
        n_edges = int(match.group(1))
    else:
        print(f"Warning: Could not extract n_edges_per_node from filename {filename}, skipping")
        continue
    ...
    # Store X_dif with key indicating n_edges_per_node value
    key_name = f"X_dif_nedges{n_edges}"
...
# Also add a metadata column to track which n_edges_per_node values were aggregated
base_adata.uns['aggregated_X_dif'] = {
    'n_edges_per_node_values': sorted([int(k.split('nedges')[1]) for k in x_dif_dict.keys()]),
...
print(f"  n_edges_per_node values: {base_adata.uns['aggregated_X_dif']['n_edges_per_node_values']}")
