"""
SCIB evaluation: benchmark different n_edges_per_node integration results using scib-metrics
"""
import sys
...
# Get all X_dif_nedges{n_edges_per_node} keys from obsm
print(f"\n=== Extracting X_dif representations ===")
x_dif_keys = [key for key in adata.obsm.keys() if key.startswith('X_dif_nedges')]
x_dif_keys = sorted(x_dif_keys, key=lambda x: int(x.split('nedges')[1]))  # Sort by n_edges_per_node value
...
if 'X_fae' in adata.obsm:
    ...
    available_keys = [k for k in adata.obsm.keys() if not k.startswith('X_dif_nedges')]
    if available_keys:
